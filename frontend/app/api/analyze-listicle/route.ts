...
export async function POST(req: NextRequest) {
  try {
    ...
    // MINIMAL TITLE FIX - just clean common pollutants from existing logic
    ...
    title = title.replace(/facebook|instagram|pinterest|twitter|search|envelope|check|plus|minus|chevron|times|rectangle/gi, '')
                 .replace(/\s+/g, ' ')
                 .trim();
    ...
    // ENHANCED AUTHOR & CONTACT EXTRACTION - Add just a few key selectors
    const quickAuthorSelectors = ['.author', '[rel="author"]', '.byline', '.author-name'];
    ...
    // ORIGINAL PROMPT - keep exactly the same
    const prompt = `
...
`;
    ...
    // Call OpenAI API with ORIGINAL settings
    ...
    // Parse JSON response - ORIGINAL logic
    let analysis;
    try {
